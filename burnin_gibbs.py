...
class burnin_gibbs(object):
    ...
    def __init__(self, num_units = 100,J = None,b = None,numpy_rng=None):
        ...
        if not J:
            ...
            # initial_J = np.asarray(numpy_rng.uniform(low = - 1.0, high=1.0, size=(num_units, num_units)) *
            #                        (np.ones((num_units,num_units)) - np.identity(num_units)))
    ...
    def sample(self,n_sample = 10000):
        independent_steps = 10 * self.num_units # this is the mix-in rate
        ...
        for i in range(sample_steps):
            ...
            E_act = np.dot(x, self.J[:,update_i[i]]) + self.b[update_i[i]]
            p_act = sigmoid(E_act)
...
if __name__ == '__main__':
    ...
    samplor = burnin_gibbs(num_units=100)
    samples = samplor.sample(n_sample=50000)
    weight = samplor.J
    bias = samplor.b

    np.save('gibbs_samples_50000.npy',samples)

    np.save('gibbs_weight_50000.npy', weight)

    np.save('gibbs_bias_50000.npy',bias)
